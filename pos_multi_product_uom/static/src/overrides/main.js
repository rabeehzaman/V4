    ...
    if (!this.uom_id || this.price_type == "manual") {
      ...
      if (this.product.uom_id[0] !== this.uom_id) {
        if (!this.do_not_update) {
          ...
          var quantity_by_factor = quantity * this.pos.units_by_id[this.uom_id].factor_inv;
          var qty = this.pos.units_by_id[this.uom_id].factor_inv;
  ...
  add_product(product, options) {
    ...
    if (self.selected_orderline) {
      if (options.unit) {
        ...
        self.selected_orderline.price_type = "manual";
        order.save_to_db()
      }
    }
    return result
