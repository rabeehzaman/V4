...
class PosPicking(models.Model):
    ...
    def _prepare_stock_move_vals(self, first_line, order_lines):
        if order_lines.product_uom_id == order_lines.uom_id :
            return {
            ...
            orderline_qty = abs(sum(order_lines.mapped('qty')))
            total_orderline_qty = orderline_qty * order_lines.uom_id.factor_inv
            return {
                'name': first_line.name,
                'product_uom': first_line.product_id.uom_id.id,
                'picking_id': self.id,
                'picking_type_id': self.picking_type_id.id,
                'product_id': first_line.product_id.id,
                'product_uom_qty': total_orderline_qty,
                'state': 'draft',
                'location_id': self.location_id.id,
                'location_dest_id': self.location_dest_id.id,
                'company_id': self.company_id.id,
